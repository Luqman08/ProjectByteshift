...
function getQuotationDetails($orderId, $excludedItemId = null)
{
    global $con;
    ...
    $query = "SELECT
                ...
                tb_product.p_price_after_markup as product_price,
    ...
    while ($row = $result->fetch_assoc()) {
        // Exclude the item with the specified ID
        if ($excludedItemId !== null && $row['product_name'] == $excludedItemId) {
            continue;
        }
...
$excludedItemId = isset($_GET["delete_item_id"]) ? $_GET["delete_item_id"] : null;

if ($orderId !== null) {
    $quotationDetails = getQuotationDetails($orderId, $excludedItemId);
} else {
...
class PDF extends FPDF
{
    ...
    function body($info, $products_info)
    {
        ...
        $textWidth = $this->GetStringWidth("QUOTATION");
        $this->Rect(10, 51, $textWidth + 15, 10, 'D'); // Adjust the coordinates and size based on the text
        ...
        $this->SetY(51);
        $this->SetX(10);
        $this->Cell($textWidth + 15, 10, "QUOTATION", 0, 1); // Adjust the width of the cell
        ...
        $grandTotal = array_sum(array_column($products_info, 'total'));

        // Display table total row
        $this->SetFont('Arial', 'B', 12);
        $this->Cell(150, 9, "GRAND TOTAL", 1, 0, "R");
        $this->Cell(40, 9, number_format($grandTotal, 2), 1, 1, "R");
    }
